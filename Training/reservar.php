...
<div class="container-fluid">
    <div class="row justify-content-center">
        <div class="col-4">
            <p class="h2 text-primary text-center mt-4 mb-4">Hola <?= $_SESSION['visitante']['nombre'] ?> - Reserva tu
                viaje</p>
            <form action="" method="post">
                <label for="viaje">Elige el viaje:</label>
                <select name="viaje" class="form-control text-center">
                    <?php foreach (getViajes() as $viaje): ?>
                        <option class="text-center"
                                value="<?= $viaje['id'] ?>"><?= $viaje['nombre'] . " (" . $viaje['precio'] . "€)" ?></option>
                    <?php endforeach; ?>
                </select>
                <label class="mt-3" for="personas">Número de personas:</label>
                ...
                <div class="mt-3 text-center form-group">
                    ...
                    <a href="viajes.php" class="btn btn-info">Listado de viajes</a>
                    <a href="reservas_realizadas.php" class="btn btn-warning">Ver reservas</a>
                </div>
                <?php if (isset($_POST['reservar'])): ?>
                    <p class="alert alert-info mt-3" role="alert"><?= $texto ?></p>
                <?php endif; ?>
            </form>
        </div>
    </div>
